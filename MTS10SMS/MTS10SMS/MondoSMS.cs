            ...
            using (StreamReader responseReader = new StreamReader(responseStream))
            {
                ...
                string resp =
                    doc.DocumentNode.Descendants("div")
                        .Where(div => div.GetAttributeValue("class", string.Empty).Contains("notice"))
                        .Select(div => div.InnerText)
                        .First();
                UserDialogs.Instance.HideLoading();
                UserDialogs.Instance.Alert(resp, "Server response", "OK");
                //UserDialogs.Instance.InfoToast("Server response: ", resp);
            }
            ...
            if (response.Headers.TryGetValues("Set-Cookie", out values))
            {
                ...
                using (StreamReader responseReader = new StreamReader(responseStream2))
                {
                    ...
                    if(doc.DocumentNode.Descendants("div").Where(div => div.GetAttributeValue("class", string.Empty).Contains("notice")).Select(div => div.InnerText).Any())
                        UserDialogs.Instance.Alert(doc.DocumentNode.Descendants("div").Where(div => div.GetAttributeValue("class", string.Empty).Contains("notice")).Select(div => div.InnerText).First(), "Server response", "OK");
                        //UserDialogs.Instance.InfoToast("Server response: ", doc.DocumentNode.Descendants("div").Where(div => div.GetAttributeValue("class", string.Empty).Contains("notice")).Select(div => div.InnerText).First());
